...
def snr_loss(output: torch.Tensor, target: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    ...
    # print(torch.max(snr), torch.min(snr))
    return -snr.mean()
...
dataset_path = "train_dataset_C_10"
...
if checkpoint== 1 and training_mode==1:
  ...
  for param_group in optimizer.param_groups:
      param_group['lr'] = 0.0001 # Schedule learning rate as needed
...
if training_mode==1:
    while epoch<n_epochs:
          ...
          for example_path in train_example_paths:
            ...
            loss = time_weight*snr_loss(output, target.to(device)) + stft_weight*torch.mean(log_error(output, target.to(device), n_fft=2**13, win_length=2**13, hop_length=2**13//2))
            loss.backward()
            optimizer.step()
            ...
            num_files +=1
          mse_loss_history.append(epoch_loss / (num_files))
          ...
          if valid==1:
            ...
            with torch.no_grad():
                    ...
                    for example_path in valid_example_paths:  # Use a separate validation dataset here
                        ...
                        val_loss = time_weight*snr_loss(output, target.to(device)) + stft_weight*torch.mean(log_error(output, target.to(device), n_fft=2**13, win_length=2**13, hop_length=2**13//2))
...
test_dataset_path = "test_dataset_C_10"
test_loss = 0
j=0
