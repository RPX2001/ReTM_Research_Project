...
if training_mode==1:
  while epoch < n_epochs:
      ...
      output_stft = output_stft.reshape(batch_size*5, freq, time_dim)  # (batch, freq * channels, time)
      target_stft = target_stft.reshape(batch_size*5, freq, time_dim)  # (batch, freq * channels, time)
      ...
      loss =  10*snr_loss(output_time, target_time) + 10*stft_error(output_stft, target_stft)
  ...
  torch.save(retm_to_save, 'retm_C.pt')


if training_mode==0 and checkpoint==1:
    model.load_state_dict(torch.load("final_checkpoint_C.pth")["model_state_dict"])
    retm_to_save = torch.load('retm_C.pt')
...
np.save("target_LSTM_C.npy", target_time.detach().cpu().numpy())
np.save("output_LSTM_C.npy", output_time.detach().cpu().numpy())  
