...
dataset_path = "train_dataset_B_10"
...
n_output_channels = 3*2
model = DepthwiseConvModel(channels, n_output_channels).to(device)
...
print(model)
# Training loop
epochs = 400#Set according to your needs
print()
valid = 0
...
for epoch in range(epochs):
    for multichannel_audio, target_audio in load_training_examples(dataset_path):
        ...
        # Forward pass
        output = model(multichannel_audio.to(device))
        target_img = torch.stft(target_audio.to(device), n_fft=n_fft, win_length=n_fft, hop_length=hop_length, window=torch.hann_window(n_fft).to(device), return_complex=False) 
        target_output = torch.cat((target_img[:,:,:,0],target_img[:,:,:,1]),dim=0)
        target_img = target_img[:,:,:,0] + target_img[:,:,:,1] * 1j # Combine real and imaginary parts
        output_img = output[0:3, :, :] + output[3:, :, :] * 1j
        ...
        loss = snr_loss(output_time, target_time) + stft_error(output_img, target_img) # Use the SNR loss function
    ...
    if valid==1:
        ...
        with torch.no_grad():
            for multichannel_audio, target_audio in load_training_examples(valid_dataset_path):  # Use a separate validation dataset here
                ...
                target_output = torch.stft(target_audio, n_fft=n_fft, win_length=n_fft, hop_length=hop_length, window=torch.hann_window(n_fft).to(device), return_complex=False) 
    ...
    print(f"Epoch {epoch+1}/{epochs} Loss: {loss.item()}", snr_loss(output_time, target_time).item(), stft_error(output_img, target_img).item())  
    if valid==1:
...
test_dataset_path = "test_dataset_B_10"
...
for multichannel_audio, target_audio in load_training_examples(test_dataset_path):
    ...
    target_audio = target_audio / max_value
    target_img = torch.stft(target_audio.to(device), n_fft=n_fft, win_length=n_fft, hop_length=hop_length, window=torch.hann_window(n_fft).to(device), return_complex=False) 
    target_output = torch.cat((target_img[:,:,:,0],target_img[:,:,:,1]),dim=0) ##[channels, freq, time]
    output = model(multichannel_audio.to(device))
...
# Assuming `log_ratio_error` and `error` are already defined
error = log_ratio_error(test_output, target_test_output)

# Initialize a NumPy array with the appropriate shape
log_ratio_error_freq = np.zeros((len(error), len(error[0])))

# Populate the array
for channel in range(len(error)):
    for i in range(len(error[channel])):
        log_ratio_error_freq[channel, i] = error[channel][i].detach().cpu().numpy()

with open("output_1.txt", "w") as file:
	for i in log_ratio_error_freq[0, :]:
		file.write(f"{i}\n")
  
with open("output_2.txt", "w") as file:
	for i in log_ratio_error_freq[1, :]:
		file.write(f"{i}\n")
  
with open("output_3.txt", "w") as file:
	for i in log_ratio_error_freq[2, :]:
		file.write(f"{i}\n")
  
np.save("target_SCONet_B.npy", target_time.detach().cpu().numpy())
np.save("output_SCONet_B.npy", output_time.detach().cpu().numpy())
