...
if checkpoint== 1 and training_mode==1:
  epoch = torch.load("final_checkpoint_denoise_B.pth")["epoch"]
  optimizer.load_state_dict(torch.load("final_checkpoint_denoise_B.pth")["optimizer_state_dict"])
  model.load_state_dict(torch.load("final_checkpoint_denoise_B.pth")["model_state_dict"])
...
if training_mode==1:
    ...
    torch.save(checkpoint, "final_checkpoint_denoise_B.pth")
...
np.save('multichannel_audio_time.npy', multichannel_audio.cpu().detach())
# mulichannel_audio = multichannel_audio / max_value
...
output_sig = output_sig.permute(1, 0, 2).reshape(3, -1).cpu()
...
torchaudio.save("denoised_sig_scenario_B.wav", denoised_sig.detach(), sample_rate)


np.save('denoised_sig_time.npy', denoised_sig.cpu().detach())
