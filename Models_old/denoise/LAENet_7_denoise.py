...
if checkpoint== 1 and training_mode==1:
  epoch = torch.load("final_checkpoint.pth")["epoch"]
  optimizer.load_state_dict(torch.load("final_checkpoint.pth")["optimizer_state_dict"])
  model.load_state_dict(torch.load("final_checkpoint.pth")["model_state_dict"])
...
if training_mode==1:
  ...
  torch.save(retm_to_save, 'retm.pt')


if training_mode==0 and checkpoint==1:
    model.load_state_dict(torch.load("final_checkpoint_lstm_autoencoder_AWGN_1100.pth")["model_state_dict"])
    retm_to_save = torch.load('retm_A_AWGN.pt')

model.eval()

test_example_paths = get_training_example_paths("test_dataset_A_AWGN")
# print(model)
...
denoising_file = "rec_scen_B.wav"

# Load the multi-channel file
waveform, sample_rate = torchaudio.load("rec_scen_B.wav")  # waveform shape: (channels, time)

# For example, extract channels 0 and 2
output_indices = [0, 1, 2]
input_indices = [3, 4, 5, 6]
...
torchaudio.save("denoised_sig_scenario_B_LSTM.wav", denoised_sig.cpu().detach(), sample_rate)
    
    

# target_stft = target_stft.reshape().squeeze()
print(result.shape, target_stft.shape)

np.save('denoised_sig_LSTM.npy', denoised_sig.cpu().detach())
np.save('multichannel_audio_LSTM.npy', multichannel_audio.cpu().detach())
